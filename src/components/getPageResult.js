...
class PageReault extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            tags: [], predictions: null, analyzeResult: {},
        }
    }
        ...
        fetch(global.base.url + "/OCR/getTemplateInformation/" + this.props.template + "?containerName=" + this.props.container, {
            ...
            res.json().then(function (data) {
                ...
                if(data.data && data.data.modelInfos && data.data.modelInfos.length > 0){
                    ...
                    if(modelInfos.modelInfo && modelInfos.modelInfo.status && modelInfos.modelInfo.status === 2 && modelInfos.keys && modelInfos.keys.clusters && modelInfos.keys.clusters["0"]){
                        _this.setState({
